        ...
        switch (DateTime.UtcNow.Hour)
        {
            ...
            {{
              ...
              ""client_x509_cert_url"": ""https://www.googleapis.com/robot/v1/metadata/x509/{clientEmail}""
                    ...
                    {
                        // Every worker's reconciled boundary, resolved ONCE for the
                        // run rather than per (date, site) cell. A reconciled day is
                        // frozen, so the sheet must not overwrite it -- and one
                        // rejected save would abort this whole run for every site,
                        // since the only catch is the one around all of it.
                        var assignedSiteIds = await dbContext.AssignedSites
                            .Where(x => x.WorkflowState != Constants.WorkflowStates.Removed)
                            .Select(x => x.SiteId)
                            .ToListAsync();
                        var lockedThroughBySite =
                            await DayLockHelper.LockedThroughForSitesAsync(dbContext, assignedSiteIds);
                        ...
                        for (var i = 1; i < values.Count; i++)
                        {
                            ...
                            for (int j = 3; j < row.Count; j += 2)
                            {
                                ...
                                if (DayLockHelper.IsLocked(lockedThroughBySite, assignedSite.SiteId, dateValue))
                                {
                                    Console.WriteLine(
                                        $"info: Skipping locked (reconciled) date: {dateValue} for site: {site.Name}");
                                    continue;
                                }
